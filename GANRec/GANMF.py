...
class GANMF(BaseRecommender):
    ...
    def fit(self, num_factors=10, emb_dim=32, epochs=300, batch_size=32, d_lr=1e-4, g_lr=1e-4, d_steps=1, g_steps=1,
        ...
        #         d_reg * tf.add_n([tf.nn.l2_loss(var) for var in self.params['D']])
        dloss = tf.maximum(0.0, m * real_recon_loss - fake_recon_loss) + self.wasserstein_disc(real_data=real_profile, fake_data=fake_profile, real_recon_loss=real_recon_loss, fake_recon_loss=fake_recon_loss, batch_size=batch_size) + \
                d_reg * tf.add_n([tf.nn.l2_loss(var) for var in self.params['D']])
